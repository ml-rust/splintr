...
- WordPiece matches through a byte trie instead of re-hashing every candidate subword, streams borrowed words instead of allocating one per word, and caches segmented words as the BPE backend does. bert-base-uncased encodes about 6x faster; allocations per encode fall from millions to thousands. Token ids are unchanged.
