...
- WordPiece matches through a byte trie instead of re-hashing every candidate subword, folds accents and case in one pass that skips the Unicode machinery for ASCII, streams borrowed words instead of allocating one per word, and caches segmented words as the BPE backend does. bert-base-uncased encodes about 4.8x faster; allocations per encode fall from millions to thousands. Token ids are unchanged.
