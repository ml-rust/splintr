...
- WordPiece matches through a byte trie instead of re-hashing every candidate subword, streams borrowed words instead of allocating one per word, borrows through the normalizer chain when a stage changes nothing, and caches segmented words as the BPE backend does. bert-base-uncased encodes about 4x faster; allocations per encode fall from millions to thousands. Token ids are unchanged.
