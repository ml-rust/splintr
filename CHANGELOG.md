...
- Merges are ranked by the ids of the pair being merged rather than by the bytes it concatenates to, so a merge costs the same whatever its operands have grown to. Non-Latin scripts gain most: their merges outgrow the byte-keyed tables immediately, Latin ones rarely do.
