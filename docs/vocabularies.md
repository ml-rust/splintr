...
| `llama2` / `llama-2` / `tinyllama` / `vicuna`          | Llama 2, TinyLlama, Vicuna, WizardLM         | 32,000                      | 3 + 54 agent    | none — SPM-BPE, no split regex         |
| `codellama` / `code_llama` / `code-llama`              | Code Llama (7B/13B/34B, base and Instruct)   | 32,016                      | 3 + 54 agent    | none — SPM-BPE, no split regex         |
| `phi4` / `phi-4`                                       | Phi-4, Phi-4-reasoning (Microsoft)           | 100,352                     | 96 + 54 agent   | `LLAMA3_PATTERN`                       |
| `olmo2` / `olmo-2`                                     | OLMo-2 (AI2)                                 | 100,278                     | 22 + 54 agent   | `LLAMA3_PATTERN`                       |
| `modernbert` / `modern-bert`                           | ModernBERT base/large, ModernBERT-Embed      | 50,368                      | 116 + 54 agent  | `GPT2_PATTERN`                         |
...
`pretrained::patterns(vocab)` returns `Option<&'static [&'static str]>`. It is `None` for Mistral V1/V2, Llama 2 and Code Llama — not "unknown", but "this vocabulary does not pre-tokenize with a regex": all four run on the SPM-BPE backend, which segments by merging pieces and never applies a split pattern.
...
**Phi-4 and OLMo-2 are cl100k_base's ranks, and neither is a cl100k_base alias.** Both state all 100,256 of them, verified rank for rank, so neither embeds vocabulary data of its own — the same arrangement gpt-oss has with o200k_base. Two things differ. Their pre-tokenizer is `LLAMA3_PATTERN`, read off their own `tokenizer.json`s, not `CL100K_BASE_PATTERN`: cl100k takes newlines one at a time (`\s*[\r\n]`) where Llama 3 takes the run (`\s*[\r\n]+`), so aliasing them would mistokenize every blank line. And their special blocks are their own — Phi-4 fills 100,256-100,351 with chat and FIM markers plus 88 `<|dummy_N|>` reservations, OLMo-2 fills 100,256-100,277 with its PII placeholders and `<|extra_id_N|>` slots. Phi-4 also declares `lstrip` and `rstrip` on all 96 of its added tokens, so a marker absorbs the whitespace on both sides: `"x <|endoftext|> y"` is `[87, 100257, 88]` on Phi-4 and `[87, 220, 100257, 379]` on OLMo-2, from the same ranks and the same marker id.

**Code Llama is Llama 2 plus 16 pieces**, extended in place: ids 0-31,999 keep Llama 2's pieces and Llama 2's scores exactly, so one file serves both and the Llama 2 vocabulary is derived from it at build time rather than committed twice. The 16 extras (`▁<PRE>`, `▁<MID>`, `▁<SUF>`, `▁<EOT>` and the fragments they build from) are not declared added tokens by either tokenizer and their scores sit far below every genuine merge, so no text produces them — a fill-in-the-middle prompt places them by id, which is what Meta's own implementation does.

**ModernBERT is the one bundled vocabulary that normalizes and the one that wraps.** It states `NFC`, applied before the split, and a `TemplateProcessing` that wraps every sequence in `[CLS]`/`[SEP]` — so `from_pretrained("modernbert")` applies both, and agrees with `from_json` on ModernBERT's own file rather than diverging from it. Every other bundled vocabulary states no template and gets none. Its added-token block is unusual too: 23 of them are runs of literal spaces, 24 down to 2, which is why indented code costs one id per indent instead of one per space. `[MASK]` declares `lstrip`, and where its reach covers a whitespace run that is itself an added token, the reference drops the whitespace token and splintr follows.
...
**Bundled vocabularies are feature-gated.** Each family has a `vocab-*` cargo feature (`vocab-cl100k`, `vocab-o200k`, `vocab-gpt-oss`, `vocab-phi4`, `vocab-olmo2`, `vocab-llama3`, `vocab-llama2`, `vocab-deepseek`, `vocab-qwen`, `vocab-glm`, `vocab-kimi`, `vocab-mistral`, `vocab-modernbert`, `vocab-whisper`), none enabled by default and all enabled in the Python wheel. Turning one off drops its data; `from_pretrained` then reports which feature is missing rather than rejecting the name. The payload lives in a `splintr-vocab-*` crate per family, so enabling one downloads one — `vocab-gpt-oss`, `vocab-phi4` and `vocab-olmo2` download nothing of their own, since each enables the family whose ranks it shares.
