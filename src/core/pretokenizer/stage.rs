...
impl Stage {
    ...
    /// Pieces are handed to `out` as they are produced rather than collected,
    /// so a chain of cutting stages can be composed by nesting sinks and needs
    /// no buffer between them at all. `dyn` rather than a generic parameter
    /// because that composition is recursive over a slice whose length is only
    /// known at run time, which a monomorphized sink cannot express.
    pub(super) fn cut<'p>(&self, piece: &'p str, out: &mut dyn FnMut(&'p str)) {
        match self {
            Stage::Split {
                matcher,
                behavior,
    ...
    pub(super) fn byte_level_for_each(
        ...
        match re {
            Some(re) => split_regex(piece, re, Behavior::Isolated, false, &mut |r| {
                emit(r, scratch)
            }),
    ...
    pub(super) fn apply_owned(&self, piece: &str, out: &mut Vec<String>) {
        match self {
            ...
            cutting => cutting.cut(piece, &mut |r| out.push(r.to_owned())),
        }
    }
}
