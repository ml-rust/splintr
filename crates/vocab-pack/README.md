...
`cargo run -p splintr-vocab-pack --example mbpe_from_json` derives an `.mbpe` from a HuggingFace `tokenizer.json`, verifying that both pack to identical bytes before it writes.
